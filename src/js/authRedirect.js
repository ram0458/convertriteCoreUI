        ...
        if (response) {
            ...
            if (response.idTokenClaims['tfp'].toUpperCase() === b2cPolicies.names.signUpSignIn.toUpperCase()) {
                ...
                if (response) {                
                    ...
                    sessionStorage.setItem('id_token', "Bearer " + response.idToken);
                    sessionStorage.setItem('userId', response.account.username);
...
function changeRole() {
    ...
    let currRole = sessionStorage.getItem('user_role');
    if (roles && currRole) {
        let curIndx;
        roles.forEach((item, idx) => {
            if (item.value == currRole) {
                curIndx = idx;
            }
        })
        if (curIndx == (roles.length - 1)) {
            curIndx = 0;
        }
        else {
            curIndx = curIndx + 1;
        }
        sessionStorage.setItem('roleid', roles[curIndx].id);
        sessionStorage.setItem('user_role', roles[curIndx].value);
